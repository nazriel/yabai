...
*layout* ['bsp|stack|h_accordion|v_accordion|float']::
...
*accordion_padding* ['<integer number>']::
    Padding used by accordion layouts to reveal each stacked window. Windows are offset by one padding unit per position in the stack, creating a cascading peek effect.
